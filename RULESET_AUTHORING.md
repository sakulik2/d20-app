...
索引与包地址必须使用 HTTPS，版本必须使用 `MAJOR.MINOR.PATCH`。`sha256` 必须是文件原始字节的 64 位十六进制摘要，不得包含 `0x`、空格或换行；可在 PowerShell 使用 `Get-FileHash -Algorithm SHA256 -LiteralPath .\my_system.json` 生成。客户端会在限长下载和写入临时文件的同时计算摘要，以常量时间比较核对索引，再解析完整 manifest、检查 ID 和版本，最后以可恢复方式替换并登记为受控安装。任一检查失败都不会安装临时文件；若受控下载文件之后无法通过语义解析，会被隔离为 `.rejected` 并自动回退同 ID 的内置 asset。客户端不提供本地导入入口。
